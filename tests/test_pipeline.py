#!/usr/bin/env python
"""
Test script to run the entire Dorian Lyrics Maker v1 backend pipeline on a sample audio file.
This script tests:
...
def run_pipeline(audio_file, lyrics_file, clean=False):
    ...
    if os.path.abspath(audio_file) != os.path.abspath(upload_audio_path):
        shutil.copy2(audio_file, upload_audio_path)
    if os.path.abspath(lyrics_file) != os.path.abspath(upload_lyrics_path):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test the Dorian Lyrics Maker v1 backend pipeline")
